...
connection_url = f"mysql+pymysql://{os.environ['TEST_DB_USERNAME']}:{os.environ['TEST_DB_PASSWORD']}@{os.environ['TEST_DB_HOSTNAME']}/lung_cancer_air_pollution_staging_area"
...
def extract_lung_cancer_data(csv_file: str):
    ...
    try:
        ...
        logger.log(logging.INFO, f"The current data has: {data.shape[0]} rows and {data.shape[1]} columns") # printing the # of rows and columns in the df for the sake of validation
        ...
        if table_exists: # if the table exists, continue to check if the data we are inserting is already there
            ...
            if existing_years != {'error'}: # if no error occurs during the data validation
                ...
                if count_of_rows_to_extract > 0: # if there is new data to extract continue as is
                    try:
                        ...
                        data_final.to_sql('lung_cancer', con= db_engine, if_exists='append', index=False)
                        ...
                        if rows_inserted == count_of_rows_to_extract:
                            session.commit()
                            logger.log(logging.INFO,f"Data inserted successfully! {count_of_rows_to_extract} rows have been inserted!")
            ...
            data.to_sql('lung_cancer', con= db_engine, if_exists='replace', index=False)
...
def extract_air_pollution_data(parquet_file: str):
    ...
    try:
        ...
        # getting the list of years in out data so we can use it in checking if the data inserted is already there or not
        ...
        logger.log(logging.INFO,table_exists)
        
        if table_exists: # if the table exists, continue to check if the data we are inserting is already there
            
            year_month = (data['year_month'].astype(str)).unique().tolist()
            ...
            if existing_years != {'error'}: # if no error occurs during the data validation
                ...
                if count_of_rows_to_extract > 0: # if there is new data to extract continue as is
                    try:
                        ...
                        data_final.to_sql('air_pollution', con= db_engine, if_exists='append', index=False)
                    ...
                    session.commit()
                    logger.log(logging.DEBUG,"No new records to extract!")
            else:
                logger.log(logging.DEBUG,"Error while checking for years!")
                session.rollback()
        
        else: # if the table does not exist continue to making the table 
            count_of_rows_to_extract = len(data)
            data.to_sql('air_pollution', con= db_engine, if_exists='replace', index=False)
