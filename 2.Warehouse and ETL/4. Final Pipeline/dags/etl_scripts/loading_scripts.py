...
def load_into_dim_treatment_types(df):
    try:
        ...
        if df is not None and isinstance(df, pd.DataFrame):
            with db_engine.begin() as cursor:
                try:
                    cursor.execute(sa.text("TRUNCATE TABLE dim_treatment_types;"))
                    ...
                    for index, row in df.iterrows():
                        cursor.execute(insert_sql, {'treatment_type': row['treatment_type']})
...
def load_into_dim_cancer_stages(df):
    try:
        ...
        if df is not None and isinstance(df, pd.DataFrame):
            with db_engine.begin() as cursor:
                try:
                    cursor.execute(sa.text("TRUNCATE TABLE dim_cancer_stages;"))
                    ...
                    # Iterate over DataFrame rows
                    for index, row in df.iterrows():
                        cursor.execute(insert_sql, {'cancer_stage_id':row['cancer_stage_id'],'cancer_stage': row['cancer_stage']})
...
def load_into_dim_air_qualities(df):
    try:       
        ...
        if df is not None and isinstance(df, pd.DataFrame):
            with db_engine.begin() as cursor:
                cursor.execute(sa.text("TRUNCATE TABLE dim_air_qualities;"))
                ...
                # Iterate over DataFrame rows
                for index, row in df.iterrows():
                    cursor.execute(insert_sql, {'air_quality':row['Category'], 'air_quality_range': row['AQI Range']})
...
def load_into_dim_smoking_status(df):
    try:
        mysql_hook = MySqlHook(mysql_conn_id=MYSQL_CONNECTION_ID)
        db_engine = mysql_hook.get_sqlalchemy_engine()
        conn = mysql_hook.get_conn()
        if df is not None and isinstance(df, pd.DataFrame):
            with db_engine.begin() as connection:
                # print(df)
                try:
                    print("Extracting smoking_status...")

                    connection.execute(sa.text("TRUNCATE TABLE dim_smoking_status;"))
                    print("Truncating dim_smoking_status table...")
                    
                    insert_sql = sa.text("""
                    INSERT INTO dim_smoking_status (smoking_status)
                    ...
                    try:
                        for index, row in df.iterrows():
                            connection.execute(insert_sql, {'smoking_status': row['smoking_status']})
                            print(f"Inserted row: {row['smoking_status']}")
                        print('dim_smoking_status laoded successfully!')
                        conn.commit()
                    except Exception as e:
                        print(e)
        ...
        if df is not None and isinstance(df, pd.DataFrame):
            with db_engine.begin() as connection:
                try:
                    ...
                    smoking_status_ids = connection.execute(sa.text("SELECT * FROM dim_smoking_status"))
                    ...
                    ss = smoking_status_ids.all()
                    ss_df = pd.DataFrame(ss, columns=smoking_status_ids.keys())
                    ...
                    result_df = df.merge(dd_df, left_on='diagnosis_date', right_on='date', how='inner') \
                                    ...
                                    .merge(ss_df, on='smoking_status', how='inner')
                    
                    final_df = result_df[['patient_id', 'patient_age', 'patient_gender', 'country_code', 'date_id_x', 'date_id_y', 'cancer_stage_id','smoking_status_id', 'hypertension', 'asthma', 'cirrhosis', 'treatment_type_id', 'survived', 'pm25_index', 'pm10_index', 'no2_index', 'so2_index', 'o3_index', 'air_quality_index', 'air_quality_id']]
                    ...
                    INSERT INTO fact_patient_air_information (
                        ...
                        cancer_stage_id, smoking_status_id, hypertension, asthma, cirrhosis, treatment_type_id, survived,
                    ...
                    for index, row in final_df.iterrows():
                        connection.execute(insert_sql, {
                            ...
                            'smoking_status_id': row['smoking_status_id'],
